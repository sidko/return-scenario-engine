...
local = {path.name: normalized_digest(path.read_bytes(), path.name) for path in Path(os.environ["DIST_DIR"]).iterdir() if path.is_file() and (path.name.endswith(".whl") or path.name.endswith(".tar.gz"))}
